    ...
    function createTable(level) {
        ...
        if (jsonView === VIEWS.AREA_SYS || jsonView === VIEWS.AREA_SRC) {
            json.children.forEach(function (childRow) {
                if (childRow.type === "partition")
                    table += parse(childRow, level, null, false);
            });
        }
        ...
        function createTableHeader() {
            ...
            columns.forEach(function (col, i) {
                ...
                if (i)
                    table_header += "value-col";
                else
                    table_header += "name-col";
                table_header += "'>";
                if (col === "Hyper-Optimized Handshaking")
                    table_header += "Hyper-Optimized<br>Handshaking"; // workaround for very long text
                else
                    table_header += col;
                table_header += "</th>";
            });
        ...
        function parse(row, level, parent, updateData, replace_name) {
            ...
            if (row.name)
                row.name = row.name.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\\n/g, '<br>');
            ...
            if (isParent) {
                ...
                if (needToSort() && jsonView !== VIEWS.SUMMARY)
                    sortedResourceChildren.sort(nameSort);
                sortedResourceChildren.forEach(function (sChild) {
                    htmlOut += sChild.html;
                });
                if (needToSort() && jsonView !== VIEWS.SUMMARY)
                    sortedOtherChildren.sort(nameSort);
        ...
        function tableRow(name, htmlClass, isParent, level, filename, line, data, shortDetails, divDetails, shorten_name) {
            ...
            if (line !== undefined && filename !== undefined && filename !== "")
                row += " onClick='syncEditorPaneToLine(" + line + ", \"" + filename + "\")'";
            else
                row += " onClick='editorNoHighlightActiveLine()'";
            ...
            if (shorten_name && jsonView !== VIEWS.SUMMARY)
                short_title = name.match(/.*\/.*:\d/i) ? name.replace(/.*\//g, '') : name;
            ...
            if (htmlClass.indexOf('parent') !== -1)
                row += "<span class=\"body glyphicon ar-toggle glyphicon-chevron-right\" aria-hidden=\"false\">&#xe080;</span>&nbsp";
            else
                row += "&nbsp&nbsp";
            ...
            if (columns && columns.last_item() === "Details")
                row += "<td class='value-col'>" + shortDetails + "</td>";
            row += "</tr>";
            return row;
        }
        ...
        function nameSort(data1, data2) {
            ...
            if (isUpper1 && !isUpper2)
                ...
                return data1.name.localeCompare(data2.name, 'en-US-u-kn-true');
        }
    }
}
...
//-------------------------------------------------------------------------------------------
/// main::parseLoopTable
function parseLoopTable(theJSON, parseLoops) {
    var loop = theJSON;
    var indent = 12;
    var htmlOut = "";
    ...
    function createRow(title, data, details, line, level, filename, resources) {
        var row = "<tr";
        var hasDetails = true;
        ...
        if (parseLoops) {
            row += " (";
            if (line > 0) {
                row += filename.substring(filename.lastIndexOf('/') + 1) + ":" + line;
            }
            else if (line === 0) {
                row += filename.substring(filename.lastIndexOf('/') + 1);
            }
            else {
                row += "Unknown location";
            }
            row += ")";
        }
        row += "</td>";
        ...
        // add details column
        if (hasDetails) {
            row += "<td class='value-col' >" + details[0] + "</td>";
        }
        else {
            row += "<td class='value-col'></td>";
        }
        // details section
        if (resources !== undefined && resources.length > 0) {
            var infohtml = "<ul class='card-title'>" + title + ":<br>";
            for (var ri = 0; ri < resources.length; ri++) {
                if (resources[ri].name !== undefined)
    ...
    function addResource(r) {
        ...
        if (r.hasOwnProperty('debug') && r.debug.length > 0 && r.debug[0].length > 0) {
            ...
            if (line > 0) {
                filename = getFilename(r.debug[0][0].filename);
            }
            else {
                filename = r.debug[0][0].filename;
            }
